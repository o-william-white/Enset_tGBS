...
# read long data
snpsift_long <- read.table("snpsift_long_eff.txt", header = TRUE, sep = "\t")
...
# read wide data
snpsift_wide <- read.table("snpsift_wide_eff.txt", header = TRUE, sep = "\t")
...
write.table(snpsift_long, "snpsift_long_eff_fmt.txt", col.names = TRUE, row.names = FALSE, sep="\t", quote = FALSE)  
write.table(snpsift_wide, "snpsift_wide_eff_fmt.txt", col.names = TRUE, row.names = FALSE, sep="\t", quote = FALSE)  
...
# save plot
png("plot_snpeff_eff.png", height=8, width = 12, units = "in", res = 600)
p
dev.off()
