...
mkdir -p /data/scratch/mpx469/tGBS_enset_project/map_reads_bedadeti/bwa_mem_${LENGTH}_output
mkdir -p /data/scratch/mpx469/tGBS_enset_project/map_reads_bedadeti/samtools_${LENGTH}_output
...
# XA - alternative hits
# SA - chimeric read
samtools view -h /data/scratch/mpx469/tGBS_enset_project/map_reads_bedadeti/bwa_mem_${LENGTH}_output/${SAMPLE}.bam | grep -v -e 'XA:Z:' -e 'SA:Z:' | samtools sort -o samtools_${LENGTH}_output/${SAMPLE}.bam -T ${SAMPLE}.${LENGTH}.tmp -

# test if all mapped reads are unique
if [ "`samtools view samtools_${LENGTH}_output/${SAMPLE}.bam |  wc -l`" == "`samtools view samtools_${LENGTH}_output/${SAMPLE}.bam | cut -f 1 | sort | uniq | wc -l`" ]; then
...
samtools index samtools_${LENGTH}_output/${SAMPLE}.bam
