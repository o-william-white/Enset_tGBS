...
rp <- read.table("populations.haps.radpainter", header = T, sep = "\t")

# read mlg input
mlg <- read.table("mlg_farthest_bitwise_monophyletic_single_rep.txt", header = T, sep = "\t")
...
# write
write.table(rp, "populations.haps.clone.correct.radpainter", col.names = T, row.names = F, quote = F, sep = "\t")
