...
# read mlg
mlg <- read.table("mlg_farthest_bitwise_monophyletic_single_rep.txt", header = TRUE)
...
# write sets
write.table(sample_metadata[,c(1,2)], "by_pop.txt",    col.names = FALSE, row.names = FALSE, quote = FALSE, sep = "\t")
write.table(sample_metadata[,c(1,3)], "by_sample.txt", col.names = FALSE, row.names = FALSE, quote = FALSE, sep = "\t")
